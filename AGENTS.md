...
- The [llms.txt](https://llmstxt.org) family is generated by `starlight-llms-txt`:
  `/llms.txt` (a curated map), `/llms-full.txt`, `/llms-small.txt`, and one subset
  per journey under `/_llms-txt/`. These are the canonical machine-readable
  surface; there is deliberately no per-page raw-Markdown endpoint.
