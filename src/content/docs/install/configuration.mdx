...
Some settings are configured with environment variables, read from a `.env` file in the
...
The Orchard app's own settings, separate from the services it connects to below.

### Required

`SETUP_KEY` is the one you must set; the rest are shown with their defaults.
Copy the block into `.env` and change what you need.
...
Every service below is optional. Orchard needs only the ones your mint actually uses,
and each one you connect unlocks the matching features in the dashboard. If your mint's
backend isn't a Lightning node, for example, you can connect just the mint and skip the
rest.

### Bitcoin

Point Orchard at the Bitcoin Core node your stack settles against, over RPC. The host
...
Manage Lightning-native assets alongside your sats mint. The host and port below are
...
This is the layer Orchard manages directly. Orchard reads the mint's database to
surface keys and balances, and talks to its RPC to manage it. Set the implementation in
**Connection**; the **Database** and **Management RPC** each configure on their own.
