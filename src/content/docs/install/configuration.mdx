...
Connect your Cashu Mint to Orchard. Orchard reaches the mint four ways:
...
- **Metrics** — to chart how the mint itself is running (cdk only).
...
<Tabs syncKey="cashu-rpc-tls">
  <TabItem label="Insecure">
    ...
    cdk-mintd with `allow_insecure = true` under `[mint_management_rpc]` and no TLS
    certificate directory; Nutshell
...
`cdk-mintd` can publish how it is running; processor and memory use, HTTP and database
activity, how long mint operations take. Point Orchard at that endpoint and it reads the
numbers every minute and charts them on the mint's
[System dashboard](/orchard/mint/#system).

```ini
MINT_METRICS_API=http://localhost:9000  # base url for mint metrics endpoint
```

<Aside type="note">
  Mint system metrics are only available for CDK mints (`MINT_TYPE=cdk`).
</Aside>
...
| `MINT_METRICS_API` | Base URL of the mint's metrics endpoint. cdk only. | `http://localhost:9000` |
