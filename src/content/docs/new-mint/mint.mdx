...
<Aside type="note" title="Keep this port private">
  The management gRPC at `127.0.0.1:8086` is reachable only from this machine, for
  Orchard, which runs here too. Keep it bound to `127.0.0.1` and never add it to the
  Cloudflare Tunnel below. Only the mint's own port (`8085`) belongs on the public
  internet.
</Aside>
...
The mint is running, and it writes to the `cdk_mintd` Postgres database and serves its
management gRPC on `127.0.0.1:8086`. The last step installs Orchard and points it at
the stack.
