...
The mint keeps its configuration and seed in a work directory. Following
...
<Aside type="danger" title="The seed is half of your backup">
  ...
  [Backup and restore](/orchard/mint/#backup-and-restore)). Lose the seed and the mint
...
<Aside type="note" title="Managing the mint through Orchard">
  ...
  shares this machine and the port is reachable only from within the network. If Orchard runs on a
...
* Back up the mint first, so you can roll back if the new version misbehaves. With
  Orchard connected, the simplest way is from its dashboard; see
  [Backup and restore](/orchard/mint/#backup-and-restore).
