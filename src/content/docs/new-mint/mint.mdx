...
description: Install a CDK Cashu mint (cdk-mintd) from the official prebuilt binary, backed by your own LND node — with Postgres, Redis, Prometheus metrics, a hardened systemd service, and a Cloudflare Tunnel to put it online. Part of the New Mint walkthrough.
---
...
Prometheus features this guide uses, so there is nothing to compile.
...
<Code code={`{"name":"Your Mint","version":"cdk-mintd/${latestCdkVersion}","description":"A Cashu mint backed by my own Lightning node","contact":[...],"nuts":{...}}`} />
...
<Aside type="tip" title="Restoring a mint">
  ...
  (`sudo -u postgres createdb -O cdk_mintd cdk_mintd`), and load your latest dump into it
...
<Aside type="note" title="Backups from Orchard">
  ...
  dashboard instead of the command line: see [Backup and restore](/orchard/mint/#backup-and-restore).
...
- **Mint information.** `name`, `description`, `contact_email`, `motd`, and `icon_url`
  ...
  [Mint information](/orchard/mint/#mint-information).
...
The mint is running, and it writes to the `cdk_mintd` Postgres database, serves its
management gRPC on `127.0.0.1:8086`, and exposes metrics on `127.0.0.1:9000`. The last
step installs Orchard and points it at all of them.

<CardGrid>
  <LinkCard
    title="Next: Install Orchard"
    href="/new-mint/orchard/"
    description="Install Orchard with its own user and point it at the stack you built."
  />
  <LinkCard
    title="CDK on GitHub"
