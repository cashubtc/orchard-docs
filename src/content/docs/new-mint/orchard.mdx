...
- **The full stack from this walkthrough running**: [system](/new-mint/system/),
  ...
  [Cashu mint](/new-mint/mint/), with the mint's management gRPC enabled (the mint step's
  config does this).
...
MINT_RPC_PORT=8086
MINT_RPC_MTLS=false
MINT_METRICS_API=http://localhost:9000
```
...
<Aside type="note" title="Mint metrics">
  `MINT_METRICS_API` points at the metrics page you turned on with `[prometheus]` in the
  [mint step](/new-mint/mint/#configuration), and the port matches the one you set
  there. It fills the mint's [System dashboard](/orchard/mint/#system). If you left the
  mint's `[prometheus]` block out, comment this line out too.
</Aside>
